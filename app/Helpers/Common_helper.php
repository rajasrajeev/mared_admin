...
if (!function_exists('has_permission')) {
    ...
    {
        // Get the logged-in user's ID if not provided
        if (empty($user_id)) {
            $user_id = auth()->id();
        }
        ...
        // Check if the user is the root admin (first registered user)
        $root_admin = App\Models\User::orderBy('id', 'asc')->first();
        if ($user_id == optional($root_admin)->id) {
            return true;
        }

        // Fetch permission IDs assigned to the user's role
        $permission_ids = App\Models\RolePermissions::where('role_id', $user->role_id)
            ->pluck('permission_id')
            ->toArray();
        ...
        // Get menu and submenu routes linked to permissions
        $menu_routes = App\Models\Menu::whereIn('id', function ($query) use ($permission_ids) {
                $query->select('menu_id')->from('permissions')->whereIn('id', $permission_ids);
            })
            ->pluck('route')
            ->toArray();

        $submenu_routes = App\Models\Submenu::whereIn('id', function ($query) use ($permission_ids) {
                $query->select('submenu_id')->from('permissions')->whereIn('id', $permission_ids);
            })
            ->pluck('route')
            ->toArray();

        // Merge all allowed routes
        $allowed_routes = array_merge($menu_routes, $submenu_routes);

        // Check if the given route is in the allowed list
...
//get admin sidebar menus and submenus contents

if (!function_exists('get_menus')) {
    function get_menus()
    {
        return Menu::orderBy('order')->get(); // Fetch all menu items ordered by 'order'
    }
}

if (!function_exists('get_submenus')) {
    function get_submenus($menuId)
    {
        return Submenu::where('menu_id', $menuId)->orderBy('order')->get(); // Fetch submenus for a specific menu
    }
}
...
if (!function_exists('get_user_permissions')) {
    function get_user_permissions()
    {
        ...
        if ($user && $user->role_id) {
            ...
            // Return permissions if the role exists
