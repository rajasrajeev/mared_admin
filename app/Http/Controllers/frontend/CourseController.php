...
class CourseController extends Controller
{
    public function index(Request $request, $category = '')
    {
        ...
        if (request()->has('price')) {
            ...
            if ($price == 'paid') {
                $query = $query->where('courses.is_paid', 1);
            } elseif ($price == 'discount') {
                $query = $query->where('courses.discount_flag', 1);
            } elseif ($price == 'free') {
                $query = $query->where('courses.is_paid', 0);
            }
        }
        ...
        if (request()->has('rating')) {
            ...
            $query  = $query->where('courses.average_rating', $rating);
        }
        ...
        $page_data['courses']  = $query->latest('courses.id')->paginate($layout == 'grid' ? 9 : 5)->appends($request->query());
