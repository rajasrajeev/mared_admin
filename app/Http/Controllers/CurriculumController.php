    ...
    public function uploadSystemVideo(Request $request)
    {
        ...
        if ($type == 'ffmpeg') {
            ...
            if (!$watermark || !file_exists(public_path($watermark))) {
                return response()->json(['error' => 'Watermark configuration missing or file not found'], 500);
            }

            $watermark_status = WatermarkController::encode($item, $file_name, $path);
            if (!$watermark_status) {
                return response()->json(['error' => 'Failed to apply watermark'], 500);
            }
        }

        $item->move($path, $file_name);
        $relative_path = 'uploads/lesson_file/videos/' . $file_name;
