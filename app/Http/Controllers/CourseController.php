...
use App\Models\CourseTypePrice;
...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['course_type']      = implode(',', $request->course_type);
    ...
    public function subjects($classId, $courseType)
    {
        $courseTypes = explode(',', $courseType);
        // dd($courseTypes);

        // Fetch subjects matching the selected course types
        $subjects = Course::where('category_id', $classId)
            ->where(function ($query) use ($courseTypes) {
                foreach ($courseTypes as $type) {
                    $query->orWhere('course_type', 'like', "%$type%");
                }
            })
            ->get();
    ...
    public function courseTypePrice(Request $request)
    {
        $classId = $request->class_id;
        $courseType = $request->course_type;

        // Find the course type price from the courseTypePrice table
        $courseTypePrice = CourseTypePrice::where(['course_type' => $courseType,'class_id' => $classId])
                                        ->first();

        if ($courseTypePrice) {
            return response()->json([
                'success' => true,
                'price' => $courseTypePrice->price
            ]);
        }
    ...
    {
        // Use pagination instead of all() to get paginated results
        $search = $request->input('search', '');

        $query = CourseTypePrice::query();
        ...
        // Join with categories table to access class title
        $query->join('categories', 'categories.id', '=', 'course_type_price.class_id')
              ->select('course_type_price.*', 'categories.title as title');

        // Get paginated results (10 per page)
        $courseTypes = $query->paginate(10);

        // Return view with course types data
        return view('admin.coursetype.index', compact('courseTypes'));
    }

    public function courseTypeCreate()
    {
        // Return the form view for creating a new course type
        return view('admin.coursetype.create');
    }

    public function courseTypePost(Request $request)
    {
        ...
        $courseType = new CourseTypePrice();
        $courseType->course_type = $validated['name'];
        $courseType->price = $validated['price'];
        $courseType->class_id = $validated['class_id'];
        // Add any other fields that need to be saved
        $courseType->save();

        // Redirect to the index page with success message
        return redirect()->route('admin.coursetype.price.index')
            ->with('success', 'Course type created successfully!');
    }
    public function courseTypeEdit(Request $request)
    {
        // Validate the incoming request to ensure we have an ID
        $validated = $request->validate([
            'id' => 'required'
        ]);

        // Fetch the course type based on the provided ID
        $courseType = CourseTypePrice::findOrFail($validated['id']);

        // Return the edit view with the course type data
        return view('admin.coursetype.edit', compact('courseType'));
    }

    public function courseTypeUpdate(Request $request)
    {
        // Validate the request data
        $validated = $request->validate([
            'id' => 'required',
        ...
        // Find the course type to update
        $courseType = CourseTypePrice::findOrFail($validated['id']);

        // Update the course type details
        $courseType->course_type = $validated['name'];
        $courseType->price = $validated['price'];
        $courseType->class_id = $validated['class_id'];
    ...
    public function courseTypeDelete($id)
    {
        try {
            // Find the course type
            $courseType = CourseTypePrice::findOrFail($id);
            ...
            // If anything goes wrong, redirect with error message
            return redirect()->route('admin.coursetype.price.index')
