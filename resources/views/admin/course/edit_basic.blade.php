<input type="hidden" name="level" value="beginner" required>
...
<div class="row mb-3">
    <label class="form-label ol-form-label col-sm-2 col-form-label">{{ get_phrase('Course type') }}<span
            class="text-danger ms-1">*</span></label>
    <div class="col-sm-10">
        @php
            $selectedTypes = explode(',', $course_details->course_type); // Convert string to array
        @endphp
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="course_type[]" value="full" id="fullCourse"
                {{ in_array('full', $selectedTypes) ? 'checked' : '' }}>
            <label class="form-check-label" for="fullCourse">
                {{ get_phrase('Full Course') }}
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="course_type[]" value="half" id="halfCourse"
                {{ in_array('half', $selectedTypes) ? 'checked' : '' }}>
            <label class="form-check-label" for="halfCourse">
                {{ get_phrase('Half Course') }}
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" type="checkbox" name="course_type[]" value="subject" id="subjectWise"
                {{ in_array('subject', $selectedTypes) ? 'checked' : '' }}>
            <label class="form-check-label" for="subjectWise">
                {{ get_phrase('Subject Wise') }}
            </label>
        </div>
    </div>
</div>
