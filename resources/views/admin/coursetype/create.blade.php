...
@push('title', get_phrase('Create Course Type'))
...
@section('content')
        ...
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                <h4 class="title fs-16px">
                    ...
                    {{ get_phrase('Create New Course Type') }}
                </h4>
    ...
    <div class="ol-card p-4">
        <div class="ol-card-body">
            <form action="{{ route('admin.coursetype.price.post') }}" method="post" class="row">
                @csrf
                <div class="col-md-12 mb-3">
                    ...
                    <input type="text" class="form-control ol-form-control @error('name') is-invalid @enderror" id="name" name="name"
                        placeholder="{{ get_phrase('Enter course type name') }}" value="{{ old('name') }}" required>
                    @error('name')
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <div class="input-group">
                        <input type="number" step="0.01" min="0" class="form-control ol-form-control @error('price') is-invalid @enderror" id="price" name="price"
                            placeholder="{{ get_phrase('Enter price') }}" value="{{ old('price') }}" required>
                        @error('price')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-12 mt-3">
                    <button type="submit" class="btn ol-btn-primary">{{ get_phrase('Create Course Type') }}</button>
                </div>
            </form>
        </div>
    </div>
