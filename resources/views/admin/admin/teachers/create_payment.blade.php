@foreach (App\Models\Payment_gateway::where('status', 1)->get() as $payment_gateway)
    ...
    @php
        ...
        if (isset($teacher)) {
            $teacher_keys = json_decode($teacher->paymentkeys, true);
        }
    ...
    @if (is_array($gateway_keys))
        @foreach ($gateway_keys as $key => $value)
            @php
                if (isset($teacher_keys) && is_array($teacher_keys) && array_key_exists($payment_gateway->identifier, $teacher_keys)) {
                    $input_val = $teacher_keys[$payment_gateway->identifier][$key];
                } else {
                    $input_val = '';
                }
            @endphp
